...
| 需求 | 通过 | 备注 |
|------|------|------|
| 1. 存储 Kline 到 PG | ✅ | KlineWriter 已集成，WsHub → mpsc → bulk insert |
| 2. 分区表设计 | ✅ | klines_phase4 分区表（202605-202610） |
| 3. 批量缓冲 | ✅ | 100 条或 5s flush |
| 4. 异步非阻塞写入 | ✅ | mpsc channel，WsHub 不阻塞 |
| 5. 历史查询 API | ✅ | get_kline handler → query_klines service |
| 6. 写入失败不阻塞 | ✅ | error log，buffer retry |
| 7. 分区管理 | ⚠️ | 可后期追加 |
| 8. 多周期支持 | ✅ | 1m/5m/15m/30m/1h/4h/1d/1w |
| 9. 自定义交易对 | ⚠️ | 依赖 symbol_configs |

**评分**: 7/9 ✅ **77.8/100**（⚠️ 为可选项或依赖外部功能）
